...
vi.mock('./action/custom-action-03.mts', () => ({
  default: vi.fn(),
}));
...
test('index.mts runs the action implementation once', async () => {
  ...
  // ASSERT
  expect(customAction03).toHaveBeenCalledTimes(1);
});

test('index.mts marks the action as failed when the implementation throws', async () => {
  // ARRANGE
  vi.mocked(customAction03).mockRejectedValueOnce(new Error('Example failure'));
  // ACT
  await import('./index.mts');
  // ASSERT
  expect(customAction03).toHaveBeenCalledTimes(1);
...
test('index.mts marks the action as failed when the implementation throws a string', async () => {
  // ARRANGE
  vi.mocked(customAction03).mockRejectedValueOnce('Example string failure');
  // ACT
  await import('./index.mts');
  // ASSERT
  expect(customAction03).toHaveBeenCalledTimes(1);
